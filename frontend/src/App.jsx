...
function ItemCard({ item, onApprove, onReject, onSimilar, showDetailLink=true }) {
  ...
  return (
    <div style={{border:"5px solid #dde2d9f8", borderRadius:12, padding:15, marginBottom:12, width: '1000px'}}>
...
function HomePage() {
  return (
    <div style={{padding:300}} >
      <h2 style={{textAlign: "center", marginBottom:16}}>Welcome to AI-Powered Exam Item Reviewer Platform</h2>
      ...
      <h4>Our unique capabilities:</h4>
        ...
        <Link to="/items">
        <div>
           <button style={{marginTop:8, backgroundColor:"#dde2d9f8"}}>Go to Items</button>
        </div>
  ...
  return (
    <div style={{padding:16}}>
      ...
      <form onSubmit={onSubmit} style={{display:"grid", gap:8, maxWidth:600}}>
        ...
        <label>
          DOCX Path
          <input value={docxPath} onChange={(e)=>setDocxPath(e.target.value)} style={{width:"100%"}} />
        </label>
        <div style={{display:"flex", gap:8}}>
          <button disabled={busy}>{busy ? "Generating..." : "Generate"}</button>
          <button type="button" onClick={()=>nav("/items")}>Back to Items</button>
        </div>
      </form>
      ...
      {result && (
        <div style={{marginTop:8}}>
          <div style={{fontSize:12, color:"#666"}}>Backend Response</div>
          <pre style={{background:"#f7f7f7", padding:8, borderRadius:8}}>
